...
date: 2026-09-11
excerpt: CLIFF Localization Suite — 把 CLIFF 1.0 接入虚幻引擎官方本地化系统，实现 .cliff ↔ UE 本地化数据的双向无损转换，并内置 DeepSeek AI 自动翻译。
featured: true
---
...
- **DeepSeek AI 翻译** — 导出 CLIFF → AI 翻译 → 导入 CLIFF 三段式管线，多语言并发、占位符校验、自翻译与空译文拒绝写回、CLIFF 自动修复闭环
- **官方入口** — Dashboard 工具栏按钮、官方同款 Slate 进度窗口、`CliffLocalizationSuite.Import/Export/AITranslate` 控制台命令

## 文档导航

### 教程

逐步引导，从安装到第一次完整跑通 CLIFF 工作流。

- **[安装与启用 →](tutorials/quick-start/installation)** — 插件安装、模块启用与验证
- **[Dashboard 工作流 →](tutorials/quick-start/dashboard-workflow)** — 第一次导出与导入 CLIFF
- **[第一次 AI 翻译 →](tutorials/quick-start/ai-translation)** — 配置 DeepSeek 并补齐缺失译文
- **[CLIFF 格式速览 →](tutorials/cliff-basics/cliff-format)** — 用 5 分钟读懂一个 `.cliff` 文件
...
- **[写回防护与修复 →](guides/ai-translation/safety)** — 自翻译、空译文、占位符与 CLIFF 修复闭环
...
- **[设置参考 →](reference/settings/settings)** — `UCliffLocalizationSuiteSettings` 全部字段
...
- **[原键侧车 →](reference/sidecar/sidecar)** — `.cliffmap.json` 结构与还原规则
...
- **[Runtime 类参考 →](api/runtime/classes)** — `FCliffDocument`、`FCliffEntry`、`FCliffSemanticMapping` 等
- **[导入器与导出器 →](api/editor/importer-exporter)** — `FCliffImporter` / `FCliffExporter` 与设置结构
- **[AI 翻译与命令let →](api/editor/ai-and-commandlets)** — `FCliffAITranslator` 与 `UGatherTextCommandletBase` 子类

...
title: "CLIFF 格式速览"
date: 2026-09-11
excerpt: "用一页读懂 CLIFF 1.0：文件骨架、header 字段、分组与继承、值形状、固定词表与校验口径。"
---
...
本页只覆盖使用插件时必需的要点。权威规范见 [cliff-format 仓库](https://github.com/Saikel-Orado-Liu)（`spec/cliff-1.0.0.md` 与 `spec/abnf/cliff-1.0.abnf`）。
...
| `dependency` | ⛔ | 依赖文件路径**列表**，如 `["../shared/terms.zh-Hans.cliff"]` |
| `x-…` | ⛔ | 扩展字段，严格校验器只警告不拒绝 |
...
> [!WARNING]
> 写出 `emotion: calm`（缺方括号）、`type: "label"`（标签加了引号）、`reference: "src/ui.cpp:42"`（列表写成标量）在 CLIFF 1.0 中都是**有效性错误**。多行文本用相邻字符串拼接或 `\n`，没有 `|` 之类的块标量语法。

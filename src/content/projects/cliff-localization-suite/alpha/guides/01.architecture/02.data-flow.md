...
| 3 | `UCliffImportCommandlet` | 读取 `SourcePath` / `ManifestName` / `ArchiveName` / `NativeCulture` / `CulturesToGenerate`，以及 `ClifFilePath`（可多个）或 `CliffDirectory` + `CliffImportCulture` |
...
- **元数据就地挂接**：同一身份（Namespace + Key + Source）不再新增重复 manifest context，而是把 `cliff.*` 挂到已存在的 context 上；否则官方 `GenerateLocRes` 会因同一 key 的多个 context 解析出冲突译文。
...
- 管线结束后，翻译好的 `.cliff` 会被拷回 `Content/Localization/<Target>/`，成为可评审、可提交的中间产物。

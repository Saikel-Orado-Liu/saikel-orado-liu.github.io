...
title: "导出管线"
date: 2026-09-11
excerpt: "CLIFF 导出如何读数据、如何分 clan、如何生成稳定 ID 与语义字段，以及原键侧车的写入规则。"
---
...
| `DestinationPath` | ✅ | 输出目录，布局为 `<DestinationPath>/<Culture>/<Clan>.cliff` |
| `CliffNamespace` | ✅ | CLIFF header 的 `namespace`（来自 Project Settings 的 `CLIFF Namespace`） |
...
| 路径 | 使用场景 | 能力 |
|:---|:---|:---|
| `FLocTextHelper::LoadAll`（manifest + 全部 culture archive） | **主路径** | 完整：源文本、译文、`cliff.*` 元数据、SourceLocation 全都在 |
...
1. **元数据**：`cliff.group` / `cliff.entry` 合法时直接沿用（保证往返稳定）
...
| CLIFF 字段 | 来源优先级 |
|:---|:---|
| `type` | ① `cliff.type` 元数据 ② `Namespace to Type` 映射表 ③ 无 → `sentence` + warning |
| `emotion` | ① `cliff.emotion` 元数据 ② `Namespace to Emotion` 映射表 ③ 无 → 按 `type` 的默认值（`dialogue`/`monologue`/`idiom` → `[neutral]`，其余 → `[objective]`） |
| `status` | ① `cliff.status` 元数据 ② 有译文 → `translated`；无译文 → `initial` |
| `context` | ① `cliff.context` 元数据 ② `SourceLocation` + `SiteDescription` + `DevNotes` 拼接 |
| `reference` | `SourceLocation`（文件:行 或 资产路径） |
| `max-width` | 仅当元数据中存在时导出（UE 没有对应来源） |
| `reviewer` | 仅当元数据中存在时导出 |

> [!NOTE]
> 官方审核模型只有「有译文且源文本匹配 = 已审」的二元语义，因此导出的 `status` 在元数据缺失时**只能**落到 `translated` / `initial`。`reviewed` / `final` 的细分完全依赖上一次导入写入的 `cliff.status` 元数据与侧车。

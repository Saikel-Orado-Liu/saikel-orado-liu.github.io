...
title: "导入管线"
date: 2026-09-11
excerpt: "CLIFF 导入到底写了什么：manifest / archive 的字段归属、cliff.* 元数据、生效源同步、空译文防护、编译与热刷新。"
---
...
`cliff.*` 元数据是**语义的唯一持久位置**：UE 本身没有 `type` / `emotion` / `status` 字段，导出时正是靠这些元数据把语义还原回 CLIFF。
...
> [!TIP]
> 导入是**幂等**的：对同一份 `.cliff` 重复导入不会产生重复条目，也不会累积重复 context。需要回滚时，用官方 Translation Editor 或重新导入旧版 `.cliff` 即可。

...
title: "写回防护与修复"
date: 2026-09-11
excerpt: "AI 译文写回前的全部防护：不覆盖人工成果、自翻译与空译文判定、占位符校验、CLIFF 自动修复闭环与失败口径。"
---
...
插件**不提供**「覆盖已有译文」的开关。要重译某条，请在 `.cliff` 里把它改回 `status: initial` 或删除 `target`——这个动作会留痕、可评审。
...
**判定**（`FCliffAITranslator::IsSelfTranslation`）：`target == source` 且源文本的 Unicode 脚本族（拉丁 / CJK / 阿拉伯 / 西里尔）与目标语言不符 → 视为自翻译，**拒绝写回**并计入失败。

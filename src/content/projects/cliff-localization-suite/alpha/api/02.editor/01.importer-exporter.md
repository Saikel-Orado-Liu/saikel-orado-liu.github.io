...
title: "导入器与导出器"
date: 2026-09-11
excerpt: "FCliffImporter / FCliffExporter 的设置结构、统计结构、方法签名与直接调用示例。"
---

# 导入器与导出器

两个 API 位于模块 `CliffLocalizationSuiteEditor`（Type: Editor，导出宏 `CLIFFLOCALIZATIONSUITEEDITOR_API`），仅在编辑器目标可用。

> [!IMPORTANT]
> 这两个入口直接操作 manifest / archive / `.locres`。**日常请走官方 GatherText 管线**（Dashboard 按钮或 `CliffLocalizationSuite.Import/Export`），它们会自动处理配置生成、子进程、报告与热刷新。直接调用适合工具链集成与自动化测试。
...
{
public:
    static bool ImportFile(const FString& InClifFilePath, const FCliffImportSettings& InSettings,
...
| 行为 | 说明 |
|:---|:---|
| 校验失败 | 返回 `false`，`OutIssues` 含全部 issue，`OutError` 为 `The CLIFF document is invalid; see the validation issues.` |
...
| 行为 | 说明 |
|:---|:---|
| 文档划分 | **每个 culture 一个文档**；header `clan` = manifest 基础名 kebab 化（`Game.manifest` → `cliff-game`） |
| 文件路径 | `<OutputDirectory>/<Culture>/<Clan>.cliff` |
...
| 侧车 | `bWriteSidecar && !OutputDirectory.IsEmpty()` 时写 `<Manifest 基础名>.cliffmap.json` |

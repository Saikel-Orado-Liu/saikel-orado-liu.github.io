...
date: 2026-09-11
excerpt: "The FCliffAITranslator API, its stats struct, self-translation detection, plus the three GatherText commandlets and the toolbox entry points."
---
...
| Repair | When `ParseAndValidate` fails after write-back and `bAutoRepairCliff` is on → up to `MaxRepairAttempts` rounds; if it still fails, **nothing is written to disk** and the file is recorded in `FailedFiles` |
...
|:---|:---|:---|:---|
| `UCliffImportCommandlet` | `CliffImport` | `SourcePath` / `ManifestName` / `ArchiveName` / `NativeCulture`, plus `ClifFilePath[]` or `CliffDirectory` | 0 on success, -1 on failure |

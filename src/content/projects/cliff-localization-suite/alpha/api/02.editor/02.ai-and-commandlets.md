...
|:---|:---|:---|:---|
| `UCliffImportCommandlet` | `CliffImport` | `SourcePath` / `ManifestName` / `ArchiveName` / `NativeCulture`，以及 `CliffFilePath[]` 或 `CliffDirectory` | 0 成功，-1 失败 |
...
| 无后缀（`ImportCliffFile` / `ExportTargetToDirectory`） | 直接调用导入器/导出器，不起子进程，适合工具与测试 |
